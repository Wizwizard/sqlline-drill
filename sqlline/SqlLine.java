...
public class SqlLine {
  ...
  public static void main(String[] args) throws IOException {
    ...
    final String fileName = "/tmp/" + args[args.length - 1];
    ...
    try{
      ...
      String[] args_final = new String[array.length + args.length - 1];
      System.arraycopy(args, 0, args_final, 0, args.length - 1);
      System.arraycopy(array, 0, args_final, args.length - 1, array.length);

	  br.close();
      is.close();
      in.close();

	  while(!(new File(fileName)).delete()){
	  	System.out.println("文件删除失败");
		Thread.currentThread().sleep(1000);
	  }

      start(args_final, null, true);
    } catch(Exception e) {
      e.printStackTrace();
    }
